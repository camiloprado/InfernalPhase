...
Layouts that already passed stay: shots (coal / tear / chip), door cell 4×2 of 256×96 and flush position, Ash wall masonry, Caim / Lilith / Bebê / hearts.

- **Doors** — locked Ember plate grows 2–4px fire tongues on the top edge. Jambs carry Wound veins and drips. Open passage stays clear, with Wound cracks and no plate.
- **Floor** — Wound field `#7A1F1A`, sparse Ember motes, 1px Ash grit, four variants. The Void-pore grit (star / nebula read) is gone. Room underlay is Wound so a missed texel is not a hole.
- **Pit** — circular Void mouth, stepped Ash rim, Bone fillet on the inner lip. Outside the rim stays opaque Void.
- **Boss FX** — `fx_beam.png` lightning bolt (CROSS / DIAG / LANES), `fx_slam.png` Bone ring (SLAM), `fx_wisp.png` Ember plume (RING). Hazard tint no longer multiplies the sheets toward orange.
