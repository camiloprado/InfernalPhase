...
Floors use `assets/sprites/env.png` (pixel tiles) over a Void fill. Doors are `doors.png` (gothic Ash arches, Bone inlay, Ember / cracked seals). **Caim** is a generated male coat-and-pistol 4×3 body (`player.png`) — not Lilith, not Bebê, not the Penitent diamond. **Lilith** is `player-female-v2` → `player_f.png`. Bebê Chorão is a difficulty **mode**; its sprite is `baby.png` (from `bebe_sprite`). Imps and the boss stay on `assets/characters/`. Cantors, wretches, the Concierge, hearts, pickups, and shots use `assets/sprites/`. If a gameplay sheet is missing, the load errors — no drawn placeholder. A Penitent-geometry player/baby atlas also fails bind (`is_character_body`).
