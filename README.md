...
Four hearts (vessel drops can raise the cap to six). Hit = a short invuln blink, not a vacation. Trash enemies drop about half the time: brimstone hearts, ember (damage-up), rare vessel (max-heart), or a shot mod (pierce / rapid / heavy / burn). Drops pulse so they read on the ash floor. Only the **current room** draws and collides its door arches; a neighbor never stamps a second frame into the shared opening. Arches sit on the 64px wall ColorRect: width = the 200px opening, depth = 167px (`WALL` 64 + `DOOR_REVEAL` 103), inset 51px from the gap center toward the room. Cell is 384×320 so the circular seal stays circular. Doors stay shut until the room is clear. Die and the floor rewinds. Beat the boss for a short ending, then restart if you want it again.
