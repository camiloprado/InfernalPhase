...
Floors use `assets/sprites/env.png` (pixel tiles) over a Void fill. Doors are `doors.png` (gothic Ash arches, Bone inlay, Ember / cracked seals). **Caim** is a ~32px top-down hooded Penitent male walker (`player.png`, 4 dirs × 4 frames, no face, Bone cloak, Ash hood, Ember brand) — not Lilith, not Bebê, not the UI diamond. **Lilith** is `player-female-v2` → `player_f.png`. Bebê Chorão is a difficulty **mode**; its sprite is `baby.png` (from `bebe_sprite`). Imps and the boss stay on `assets/characters/`. Cantors, wretches, the Concierge, hearts, pickups, and shots use `assets/sprites/`. If a gameplay sheet is missing, the load errors — no drawn placeholder. Bind fails if Caim is diamond / female / Bebê (`is_caim_own_body`).
