...
Four hearts (vessel drops can raise the cap to six). Hit = a short invuln blink, not a vacation. Trash enemies drop about half the time: brimstone hearts, ember (damage-up), rare vessel (max-heart), or a shot mod (pierce / rapid / heavy / burn). Drops pulse so they read on the ash floor. Only the **current room** draws and collides its door arches; a neighbor never stamps a second frame into the shared opening. Arches sit on the 64px wall ColorRect: width = the 200px opening, depth = 100px (`WALL` 64 + `DOOR_REVEAL` 36), inset 18px from the gap center toward the room. Cell is 384×192 so the circular Ember / cracked seal stays circular. The Threshold pit is a Void-deep disk with a thick Ash rim and a thin Bone hairline (not a white hoop). Doors stay shut until the room is clear. Die and the floor rewinds. Beat the boss for a short ending, then restart if you want it again.
